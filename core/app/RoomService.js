...
const RoomService = {
    ...
    createRoom: async function(playerId){
        ...
        if(isLogin){
            ...
            let data = _.zipObject([playerId], [{ id:playerId, roomId:roomId, status: 1, score: 0, isHomeOwner: true,pos: 0,
    ...
    joinRoom: async function(roomId, playerId){
        ...
        let data = _.assign(roomInfo,  _.zipObject([playerId], [{ id:playerId, roomId:roomId, status: 0, score: 0, isHomeOwner: false,pos: playerCount, roomRule: 0, avatar: user.avatar,name: user.name,isHint: true}]));
        this.rooms = DataHelper.setRoomsInfo(roomId, this.rooms, data);
        // 设置玩家房间号 和 玩家位置
