...
const PlayerManager = require("../../core/app/PlayerManager");
...
user.post('/login', async ctx =>{
	...
	if(!data || !data.account) {
		...
		let roomId = PlayerManager.getRoomId(playerId);
		let UserGameInfo;
		if(roomId){  //如果用户在房间内
			let room = _.get(RoomService, `rooms[${roomId}]`);
			if(room){  //且房间还存在
				UserGameInfo = PlayerManager.getUserGameInfo(playerId);
			} else{  //房间不存在，同步清除用户的房间信息
				PlayerManager.cleanUserStatus(playerId);
			}
			let result = { userInfo, GameServerInfo,UserGameInfo };
			response = Validate.checkSuccess("用户还在房间内", Errors.SUCCESS, HttpStatus.OK, result)
		} else { //清除用户所有房间状态及登录状态
			let result = { userInfo, GameServerInfo,UserGameInfo };
			if (userInfo && GameServerInfo) {
				PlayerManager.setIsLogin(playerId,true);
